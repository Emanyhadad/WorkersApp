                                        ...
                                        .addOnSuccessListener( documentSnapshot1 -> {
                                            if ( documentSnapshot1.exists( ) ) {
                                                ...
                                                Glide.with( PostActivity2.this )
                                                        .load( image ).circleCrop( ).error( R.drawable.worker ).into( binding.imgIWJWorker ); } } ).addOnFailureListener( e -> { } );
                                ...
                                binding.ApBtnFinishJob.setOnClickListener( view -> {
                                            new AlertDialog.Builder( PostActivity2.this )
                                                    ...
                                                    .setPositiveButton( "نعم ، اكمال" , ( dialogInterface , i ) -> {
                                                                evaluationDialog.show( ); } ).create( ).show( );
...
//                                                    binding.ratingBarWorker.setProgress((int) ratingWorker*2);
                            ...
                            case "done":
                                ...
                                firestore.collection( "users" )
                                        ...
                                        .addOnSuccessListener( documentSnapshot1 -> {
                                            if ( documentSnapshot1.exists( ) ) {
                                                String fullName = documentSnapshot1.getString( "fullName" );binding.tvIWJWorkerName.setText( fullName );
                                                String image = documentSnapshot1.getString( "image" );
                                                Glide.with( PostActivity2.this ).load( image ).circleCrop( ).error( R.drawable.worker ).into( binding.imgIWJWorker ); }
                                        } ).addOnFailureListener( e -> { } );
                        ...
                        firestore.collection( "offers" ).document( Objects.requireNonNull( postId ) )
                                ...
                                .get( ).addOnCompleteListener( task -> {
                                    if ( task.isSuccessful( ) ) {
                                        int count = task.getResult( ).size( );binding.APbtnComments.setText( count == 0 ? "0" : String.valueOf( count ) ); } } );
                        ...
                        if ( hours > 0 ) { timeDifference = "قبل " + hours + "  ساعة"; }
                        else if ( minutes > 0 ) { timeDifference = "قبل " + minutes + "  دقيقة"; }
                        else { timeDifference = "قبل " + seconds + " ثانية"; }
...
//    }
    ...
    void VisibleAndGone(String _jobState){
        switch (_jobState ){
            case "open" :
